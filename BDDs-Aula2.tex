...
\begin{frame}{Principais algoritmos para ROBDDs}
    ...
    \begin{outline}
        \1 \textproc{reduce}. Permite reduzir OBDDs de forma eficiente
            \2[-] consiste basicamente da aplicação das simplificações C1-C3
            
        \1 \textproc{apply}. Permite realizar as operações lógicas $\land$, $\lor$ e $\lnot$ (via $\phi \oplus 1$)
...
%-------------------------------------------
\begin{frame}{Algoritmo \textproc{reduce}}
...
%-------------------------------------------
\begin{frame}{Ilustração do algoritmo \textproc{reduce}}
...
%-------------------------------------------
\begin{frame}{Algoritmo \textproc{apply}}
...
\begin{frame}{Dependência conceitual}
    ...
    \centering
    O algoritmo \textproc{apply} utiliza o conceito da \underline{Expansão de Shannon}
...
%-------------------------------------------
\begin{frame}{Uso no algoritmo \textproc{apply}}
    ...
    \begin{center}
        \ovalbox{
            \begin{minipage}[h]{0.9\columnwidth}
                \noindent\scriptsize
                E, no algoritmo \textproc{apply}, é usada para eliminar as variáveis com a realização de restrições
            \end{minipage}
        }
    \end{center}
...
\begin{frame}{Programação dinâmica}
    ...
    \begin{outline}
        \1 O algoritmo \textproc{apply} também utiliza programação dinâmica para melhorar a eficiência
...
%-------------------------------------------
\begin{frame}{Ilustração do algoritmo \textproc{apply}}
...
\begin{frame}{Outros algoritmos para ROBDDs}
    ...
    \begin{outline}
        \1 \textproc{restrict}. Permite eliminar variáveis em diagramas
            
        \1 \textproc{exists}. Permite utilizar quantificadores em expressões
    \end{outline}
...
%-------------------------------------------
\begin{frame}{O algoritmo \textproc{restrict}}

    \small
    O algoritmo \textproc{restrict} calcula o ROBDD que representem $\phi[0/p]$ ou $\phi[1/p]$. É bem simples, e funciona assim:
    ...
    \begin{outline}
        ...
        \1 Então, o algoritmo \textproc{reduce} é chamado para simplificar o OBDD resultante
...
\begin{frame}{O algoritmo \textproc{exists}}
    ...
    \small
    O algoritmo \textproc{exists} representa expressões em termos de subconjuntos de restrições. Funciona assim:
