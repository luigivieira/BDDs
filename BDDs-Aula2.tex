...
\begin{frame}{Algoritmo de redução}
    ...
    \begin{outline}
        \1 Percorre os níveis de $B$ de baixo para cima
            \2[-] {\small isto é, com  busca em profundidade}
            
        \1 Atribui o mesmo identificador a nós que denotam a mesma função
            \2[-] {\small compartilhando subdiagramas (simplificações C1/C3)}
        
        \1 Atribui o identificador dos filhos um nó se eles forem os mesmos
            \2[-] {\small ignorando nós redundantes (simplificação C2)}
    \end{outline}
...
\begin{frame}{Pseudocódigo de \textproc{reduce}}
    \begin{algorithm}[H]
        ...
        \begin{algorithmic}[1]
            ...
            \Function{reduce}{$n$}
                \If{$n = 0 \lor n = 1$} \Comment simplificação C1
                    \State \Return \Call{getnode}{n,\textproc{null},\textproc{null}}
                \EndIf
